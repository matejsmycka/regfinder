...
function WakeOnLanPCs
{
	...
	foreach ($computer in $computers)
	{
		...
		Start-Job -Name $computer -ArgumentList @($computer, $broadcastAddress, $author, $function) -ScriptBlock {
			...
			if ($macAddress -eq $null -or $macAddress -notmatch "(..):(..):(..):(..):(..):(..)")
			{
				...
				MysqlQueryInsert -query "INSERT INTO logs (id, computer_name, process, status, first_check, last_check, note1, note2, author) VALUES('', '$computer', '$function', 'FAILED', '$first_check' , '$last_check', 'Nespravna MAC adresa', '', '$author' )"
			...
			else
			{
				...
				if(!(Test-Connection -ComputerName $computer -Count 2)){
				    ...
				    {
					    Logger -type "ERROR" -text "Pocitac $computer sa nepodarilo zapnut..."
					    MysqlQueryInsert -query "INSERT INTO logs (id, computer_name, process, status, first_check, last_check, note1, note2, author) VALUES('', '$computer', '$function', 'FAILED', '$first_check' , '$last_check', 'Pocítac sa nepodarilo zapnút', 'Pocet odoslaných magic paketov je $j', '$author' )"
				    }
				    else
				    {
					    Logger -type "INFO" -text "Pocitac $computer je zapnuty..."
					    MysqlQueryInsert -query "INSERT INTO logs (id, computer_name, process, status, first_check, last_check, note1, note2, author) VALUES('', '$computer', '$function', 'OK', '$first_check' , '$last_check', 'Pocítac sa podarilo zapnút vo vyhradenom casovom limite', 'Pocet odoslaných magic paketov je $i', '$author' )"
				    }
                }
                else
                {
                    Logger -type "INFO" -text "Pocitac $computer bol zapnuty pred spustenim skriptu..."
					MysqlQueryInsert -query "INSERT INTO logs (id, computer_name, process, status, first_check, last_check, note1, note2, author) VALUES('', '$computer', '$function', 'OK', '$first_check' , '$last_check', 'Pocítac bol zapnuty pred spustenim skriptu', 'Pocet odoslaných magic paketov je 0', '$author' )"
                }
			}
		}
	}
