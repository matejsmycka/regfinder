...
#Start-Transcript -Path "C:\pcsm_backend\TestEnvironment\transcriptLog\$filename.txt" -Append

#DB config
# PCSM database connection string
$connectionString = "server=tali.ics.muni.cz;uid=opsi-reader;pwd=CobDyhenJoc5;database=pcsmdev;"
...
function WakeOnLanPCs
{
    ...
    get-job | foreach{
	    $jobFilename = "C:\pcsm_backend\TestEnvironment\transcriptLog\wake_" + $_.Name + "_" + $frontaID + ".txt"
...
function Logger
{
	...
	$logName = "C:\pcsm_backend\TestEnvironment\logs\" + $date + "_log.txt"
	echo $logname
...
function getDhcpMac(){
    ...
    switch($domain){
        ...
        "staff.ucn.muni.cz" {$serverName = "staff-server2.staff.ucn.muni.cz"}
